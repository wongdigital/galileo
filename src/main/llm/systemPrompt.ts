...
LINKING: Whenever you name an event a tool returned this turn, wrap its title in Markdown bold — **exactly the title string from the tool result, nothing else inside the asterisks**. The app turns each bolded title that matches a returned event into a clickable link to that event's card; a paraphrased, truncated, or unbolded title stays dead text. Times and rooms go outside the bold ("**Star Wars: A Retrospective** — Thu, Jul 23, 10:00 AM"). Facet values may reach you as machine ids ("star-wars-lego") with a human label alongside — quote the label in prose, never the id.`
